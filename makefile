DOCKERHUB_USER = johnsondnz
REPONAME = container-ansible
DOCKERHUB = $(DOCKERHUB_USER)/$(REPONAME)
GITHUB_USER = johnsondnz
...
BUILDFLAGS = \
  ...
  --label org.label-schema.docker.cmd="docker run -it --rm $(CONTAINERNAME) <playbook>.yml -i <inventory> <flags>" \
  --label org.label-schema.docker.volume.ssh-private="-v ~/.ssh/id_rsa:/root/.ssh/id_rsa" \
  --label org.label-schema.docker.volume.ssh-public="-v ~/.ssh/id_rsa.pub:/root/.ssh/id_rsa.pub" \
  --label org.label-schema.docker.volume.playbook="-v dir:/ansible/playbooks" \
  --label org.label-schema.docker.volume.logs="-v /tmp/log:/var/log/ansible/" \
